...
export const Modal = (props: any) => {
  const stupidPostgresInconsistencyTrue = import.meta.env.NODE_ENV === 'development' ? 1 : true
  const stupidPostgresInconsistencyFalse = import.meta.env.NODE_ENV === 'development' ? 0 : false
  const showHideClassName = !props.show ? "modal display-none" : "modal display-flex";
  const { email } = props;
  let buttonEnabled = false;
  const url =
    import.meta.env.REACT_APP_backendURL || 'https://labs10-cleaner-app-2.herokuapp.com'
  const [data, err, loading] = useFetch(`${import.meta.env.REACT_APP_backendURL}/surveys`, true, 'get');
